...
function Vehiculos() {
    ...
    const renderVehiculoCard = (vehiculo) => (
        <div key={vehiculo.id} className="carrusel-card">
            ...
            <div className="carrusel-content">
                <div className="carrusel-header-info">
                    ...
                    <div className="carrusel-categorias">
                        {(Array.isArray(vehiculo.categoria) ? vehiculo.categoria : (vehiculo.categoria ? [vehiculo.categoria] : [])).map((c, i) => (
                            <span key={i} className={`categoria-badge ${(c === 'Híbrido/Eléctrico' || vehiculo.combustible === 'Eléctrico') ? 'categoria-badge--eco' : ''}`}>
                                {c}
                            </span>
                        ))}
                    </div>
                ...
                <div className="carrusel-features">
                    ...
                    <div className={`feature ${((Array.isArray(vehiculo.categoria) ? vehiculo.categoria : [vehiculo.categoria]).includes('Híbrido/Eléctrico') || vehiculo.combustible === 'Eléctrico') ? 'feature--eco' : ''}`}>
    ...
    return (
        <div className="vehiculos-page">
                ...
                <div className="filter-bar">
                    <div className="filter-left">
                        ...
                        <div className="filter-pasajeros">
                            <span className="filter-label filter-label--ml">Pasajeros:</span>
            ...
            <div className="vehiculos-por-categoria-container">
                {vehiculosFiltrados.length === 0 ? (
                    <div className="vehiculos-empty">
                        <h3>No se encontraron vehículos disponibles con los filtros actuales.</h3>
                        <button className="action-btn action-btn--mt" onClick={() => { setCategoriaFiltro('Todos'); setSearchTerm(''); setPasajerosFilter('Todos'); }}>Limpiar filtros</button>
                    </div>
                ) : (
                    categorias.map(cat => {
                        ...
                        return (
                            <section key={cat.nombre} id={`categoria-${cat.nombre}`} className="carrusel-section">
                                <div className="carrusel-header">
                                    <div className="carrusel-cat-icon">
                                        {cat.icono}
                                    </div>
                                    <div>
                                        <h2>{cat.nombre}</h2>
                                        <p>{vehiculosEnEstaCat.length} modelos disponibles para reservar en esta categoría</p>
                                    </div>
                                </div>

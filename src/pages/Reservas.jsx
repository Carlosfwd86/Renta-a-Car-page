    ...
    const handleSiguiente = () => {
        ...
        if (step === 2) {
            if (!clientName || !clientEmail || !clientEmailConfirm || !clientId || !clientPhone || !clientAddress || !clientIdFile) {
                alert("Por favor, completa todos los campos del formulario y adjunta tu identificación.");
                return;
            }
            ...
            <main className="reservas-process-container">
                ...
                <VehicleCarousel
                    ...
                    onSelectVehicle={(v) => {
                        ...
                        document.getElementById('vehicle-select-group')?.scrollIntoView({ behavior: 'smooth' });
                    }}
                />
                ...
                {step === 1 && (
                    <div className="reservas-step-card">
                        <h2>1. Selecciona tu Vehículo</h2>
                        <div className="reservas-form-grid">
                            <div className="r-input-group" id="vehicle-select-group">
                                <label>Vehículos Disponibles</label>
                                <select
                                    value={vehiculoSeleccionado?.id || ''}
                                    onChange={(e) => setVehiculoSeleccionado(vehiculos.find(v => v.id === e.target.value))}
                                >
                                    <option value="">-- Elige un vehículo --</option>
                                    {vehiculos.map(v => (
                                        <option key={v.id} value={v.id}>{v.marca} {v.modelo} (${v.precioDia}/día)</option>
                                    ))}
                                </select>
                            </div>
                        ...
                        {vehiculos.length === 0 && !cargando && (
                            <div className="no-disponible-warning">
                                <FaExclamationTriangle />
                                <p>Vehículo no disponible en estas fechas. Por favor, selecciona otro de nuestra flota.</p>
                            </div>
                        )}
                                ...
                                <div className="r-phone-input-wrapper">
                                    <select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setClientPhone(""); // Reset al cambiar país
                                        }}
                                    >
                                        {paisesData.map(p => (
                ...
                {step === 4 && (
                    <div className="reservas-step-card" style={{ textAlign: 'center' }}>
